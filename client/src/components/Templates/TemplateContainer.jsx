...
export function TemplateContainer() {
  ...
  async function handleSubmit(event) {
    ...
    if (mutationResponse) setIsModalOpen(!isModalOpen);
  }

  return (
    <div className="mx-auto md:ml-32 my-10">
      <div className="flex gap-5">
        <h3 className="text-primary font-extrabold text-3xl">Your Templates</h3>
        <button
          onClick={() => setIsModalOpen(!isModalOpen)}
          type="button"
          className="flex items-center font-medium rounded-lg text-md px-5 py-2.5 gap-1 text-primary bg-primary_faded bg-opacity-40  focus:outline-none focus:ring-0 text-center"
        >
          <AiOutlinePlus style={buttonStyle} size={24} /> Template
        </button>
      </div>

      <div className="w-9/12 grid grid-cols-1 lg:grid-cols-2 gap-5 mt-14">
